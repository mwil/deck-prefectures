...
def prepare_images(wd_df):
    ...
    for idx, row in wd_df.iterrows():
        for itype in ('flag', 'seal'):
            ...
            conv_type = ('symbol' if itype == 'seal' and not row['svg_seal']
                         else itype)

            pngpath = convert_img(
                row['name_en'], itype, row[f'svg_{conv_type}'])

            wd_df.loc[idx, f'pngpath_{itype}'] = pngpath
            wd_df.loc[idx, f'img_{itype}'] = (
                f'<img src="{pngpath.name}" height="128">' if pngpath else None)
...
def prepare_anki(wd_df):
    ...
    for name in ('flag', 'seal'):
        media_files.extend(
            map(str, wd_df[f'pngpath_{name}'].dropna().tolist()))
    ...
    for _, reg_group in wd_df.groupby('reg_name_en', sort=False):
        reg_data = {
            ...
            'stats_population_density':
                f'{reg_group.iloc[0]["reg_stats_population_density"]:.2f}',
            'stats_population_rank':
                str(reg_group.iloc[0]['reg_stats_population_rank']),
...
def prepare_states(wd_df):
    ...
    wd_df = wd_df.astype(
        {'stats_population': 'int', 'stats_area': 'float'})

    pop_dens = (wd_df['stats_population'] / wd_df['stats_area'])

    wd_df = wd_df.assign(
        stats_population_density=pop_dens.apply(lambda x: f'{x:.2f}'))

    wd_df = wd_df.sort_values(
        by='stats_population', ascending=False, ignore_index=True)
...
def prepare_regions(wd_df):
    ...
    for reg_indices in wd_df.groupby('reg_name_en').groups.values():
        wd_df.loc[reg_indices, 'reg_stats_population'] = (
            wd_df.loc[reg_indices, 'stats_population'].sum())

        wd_df.loc[reg_indices, 'reg_stats_area'] = (
            wd_df.loc[reg_indices, 'stats_area'].sum())
    ...
    pop_indices = regions.sum().sort_values(
        by='stats_population', ascending=False).index
    ...
    for rank, pop_index in enumerate(pop_indices, 1):
        ...
        wd_df.loc[reg_index, 'idx'] = (
            100*rank + 2*np.arange(len(reg_index)) + 1)

    area_indices = regions.sum().sort_values(
        by='stats_area', ascending=False).index
    ...
    wd_df =  wd_df.astype({
        'idx': int,
        'reg_stats_population_rank': int,
        'reg_stats_area_rank': int
    })
